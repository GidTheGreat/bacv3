...
export async function upload(payload){
    postMessage({
                store:"appStore",
                notification:`[UPLOADED FILE] Received:${payload.file.name}`
            })

    //console.log("beginning extraction")
    let zip = await JSZip.loadAsync(payload.file);
    //console.log(zip)

    let csv = await zip.file(payload.csvName ? payload.csvName : payload.file.name.replace(".zip",".csv")).async("string");
    let m = Papa.parse(csv, {
        header: true,
        dynamicTyping: true,
        skipEmptyLines: true,
    ...
    postMessage({
            store:"appStore",
            notification:`[UPLOADED FILE]CSV Parsed into JSON File:${payload.file.name}`
        })
    const length=m.length;

    const fileDate = ()=>{
        return payload.file.name.replace(".zip","").split("-").slice(2).join("-")
    }
    const bufferKey = `${payload.exchange}|${payload.market}|${payload.file.name.split("-")[0]}|${fileDate()}`
    const chartKey = `${payload.exchange}|${payload.market}|${payload.file.name.split("-")[0]}`
    ...
    postMessage({
            store:"appStore",
            notification:`[UPLOADED FILE]Generating Candles, File:${payload.file.name}`
        })
